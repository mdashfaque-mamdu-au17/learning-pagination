  ...
  return (
    <>
      {noOfPages > 1 ? (
        <div className="">
          <div className="">
            <button className="" onClick={onPrevPage} disabled={!canGoBack}>
              &#8249;
            </button>
            {pagesArr.map((num, index) => (
              <button onClick={() => onPageSelect(index + 1)} className="">
                {index + 1}
              </button>
            ))}
            <button className="" onClick={onNextPage} disabled={!canGoNext}>
              &#8250;
            </button>
          </div>
        </div>
